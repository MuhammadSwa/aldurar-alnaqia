...
class PlayAudioBtnZikrPage extends StatelessWidget {
  ...
  final audioController = Get.find<Controller>();
    final downloadController = Get.find<DownloaderController>();

  downloadController.ensureKnown(id, DownloadType.narrations);
    ...
    return Obx(() {
      ...
      if (isFileDownloaded) {
        return IconButton(
          onPressed: () => audioController.initPlayer(url!, title, true, localId: id),
      ...
      builder: (dialogContext) {
        return StreamOrDownloadDialog(
          ...
          onStream: () {
            ...
            audioController.initPlayer(url!, title, false, localId: id);
          },
          onDownload: () {
            Navigator.of(dialogContext).pop();
