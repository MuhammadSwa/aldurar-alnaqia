...
class _PdfviewerWidgetState extends State<PdfviewerWidget>
  ...
  StreamSubscription? _resumeToastSubscription;
  ...
  void initState() {
    ...
    _resumeToastSubscription = controller.resumedFromPage.listen((page) {
      if (page != null && mounted) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (!mounted) return;
          final text = 'تمت المتابعة من الصفحة $page';
          ScaffoldMessenger.of(context)
            ..removeCurrentSnackBar()
            ..showSnackBar(SnackBar(content: Text(text)));
          controller.resumedFromPage.value = null;
        });
      }
    });
  }
  ...
  _resumeToastSubscription?.cancel();

    // Ensure we persist state and clean listeners when widget is disposed
    controller.detachPageChangeListener();
    // Trigger GetX controller lifecycle onClose to save page and cleanup
    if (Get.isRegistered<PdfViewerGetxController>(tag: widget.title)) {
      Get.delete<PdfViewerGetxController>(tag: widget.title, force: true);
    }
