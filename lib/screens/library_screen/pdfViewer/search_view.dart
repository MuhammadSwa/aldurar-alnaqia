...
class _SearchResultTileState extends State<SearchResultTile> {
  ...
  TextSpan createTextSpanForMatch(
      PdfPageText? pageText, PdfPageTextRange match,
      {TextStyle? style}) {
    style ??= const TextStyle(
      fontSize: 14,
    );
...
class PdfPageTextCache {
  ...
  final _loadingFutures = <int, Future<PdfPageText>>{};
  ...
  Future<PdfPageText> loadText(int pageNumber) async {
    ...
    // Coalesce concurrent loads for the same pageNumber
    if (_loadingFutures.containsKey(pageNumber)) {
      final pageText = await _loadingFutures[pageNumber]!;
      final existing = _pageTextRefs[pageNumber];
      if (existing != null) {
        existing.refCount++;
        return existing.pageText;
      }
      final newRef = _PdfPageTextRefCount(pageText);
      _pageTextRefs[pageNumber] = newRef;
      newRef.refCount++;
      return newRef.pageText;
    }

    final future = textSearcher
        .loadText(pageNumber: pageNumber)
        .then((value) => value!);
    _loadingFutures[pageNumber] = future;
    try {
      final pageText = await future;
      final newRef = _PdfPageTextRefCount(pageText);
      _pageTextRefs[pageNumber] = newRef;
      newRef.refCount++;
      return newRef.pageText;
